...
foreach ($data as $station) {
	$latitudArray = explode(", ", $station["coordenadas"]);//[5.9757784064,164.2884796416]
	$latitud = $latitudArray[0];
	if ($station["numberrange"] < -20 || $station["numberrange"] > 40)
		echo "Extreme temp. station: " . $station["estacion"] . "<br>";
	if ($latitud < 0) {
		$totalTemperature += $station["numberrange"];
		$totalSouth++;
	} else {
		$totalNorth++;
	}
}
echo "Existen $totalNorth estaciones en el Hemisferio Norte<br>";
echo "La temperatura media en el Hemisferio Sur es de " . ($totalTemperature / $totalSouth) . "ºC<br>";
