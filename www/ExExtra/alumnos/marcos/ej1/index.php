...
foreach ($data as $estacion) {
    ...
    $temp = floatval($estacion['numberrange']);
    ...
    if ($temp < -20 || $temp > 40) {
        $extrema[] = $estacion['estacion'];
    }
}

$media_sur = array_sum($temp_sur) / count($temp_sur);

echo "<h1>Informacion metereologica</h1>";
echo "Número de estaciones del Hemisferio Norte: " . $contador_norte . "\n";
echo "<br>";
echo "Temperatura media de las estaciones del Hemisferio Sur: " . $media_sur . "°C\n";
echo "<br>";
echo "Estaciones con temperaturas extremas:\n";

foreach ($extrema as $estacion_name) {
    echo " - " . $estacion_name . "\n";
}
