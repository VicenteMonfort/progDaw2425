...
$gorditos = sobrePeso($data);
$total = count($data);
$porc = $gorditos*100/$total;

echo "En total hay ", $gorditos, " perritos con fucking panza";
echo "<br> Son un total del ", $porc, "%";


function edadHumana($edadCan): float{
    return (31+16*log($edadCan));
}

function sobrePeso($arrayCan): int{
    $panzas = 0;
    foreach($arrayCan as $chucho){
